...
============================================
BULK INSERT DATA FROM BRONZE TO SILVER LAYER
============================================
	...
	BEGIN TRY 
		...
		SELECT
			...
			CAST (
				...
				AS DATE
			) AS prd_end_dt -- Calculate end date as the day before the next start date 
		...
		SELECT
			...
			END AS sls_due_dt, -- Converting integer date to proper date format and handling invalid dates
			CASE 
		...
		SELECT
			...
			CASE 
				...
				ELSE TRIM(cntry)
			END AS cntry -- Normalize and handle missing blank country values and remove unwanted spaces
